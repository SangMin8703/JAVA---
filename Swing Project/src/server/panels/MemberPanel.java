...
import jdbc.model.*;
import server.dialogs.SendMessageDialog;
...
public class MemberPanel extends JPanel {
	...
	public String getSelectedMember_Id() {
		if( tableDataList == null || tableDataList.size() == 0 )
			return null;
		
		int selectionRowNumber = table.getSelectedRow(); 
		if( selectionRowNumber == -1 )
			return null;
		
		return tableDataList.get(selectionRowNumber).getMember_id();
	}
	...
	private void setEventListener() {
		this.table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				int selectionRowNumber = table.getSelectedRow(); 
				String member_id = table.getValueAt(selectionRowNumber, 0).toString();
				
				if( !member_id.equals(parents.getLoginID()) )
					parents.setMemberFileDataTable(member_id);
			}		
		});
		...
		this.table.addMouseListener(new MouseAdapter() {
			public void mouseClicked(MouseEvent e) {
				...
				SendMessageDialog messageDialog = 
						new SendMessageDialog(parents.getInstance(), info);
