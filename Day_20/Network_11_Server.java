...
public class Network_11_Server extends JFrame {
	...
	class SendProcListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {	
			...
			// 모든 클라이언들을 소켓 형태로 보관하기 때문에 매번 스트림을 생성하고 종료합니다.
			// (하지만 스트림의 종료 시 클라이언트의 접속이 종료되는 문제점을 내포하고 있습니다.)
			// (클라이언트 측에서 입력스트림 사용하여 계속해서 대기하는 경우 서버가 스트림을 종료할때
			//  클라이언트의 입력 스트림이 종료되어 소켓이 close 됩니다.)
			for( int i = 0 ; i < clients.size() ; i++ ) {
				...
				try {
					...
					textArea.append(
						inet.getHostAddress() + " 클라이언트에게 메세지 전달 실패\n");
				} finally {
					if( out != null ) {
						out.close();
					}
